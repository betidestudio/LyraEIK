...
bool UCommonUserSubsystem::AutoLoginOSSv1(FOnlineContextCache* System, TSharedRef<FUserLoginRequest> Request, FPlatformUserId PlatformUser)
{
	if(UEIKSettings* EIKSettings = GetMutableDefault<UEIKSettings>())
	{
		...
		case EAutoLoginTypes::None:
			AccountCredentials.Type = TEXT("");
			break;
		case EAutoLoginTypes::AccountPortal:
			AccountCredentials.Type = TEXT("accountportal");
			break;
		case EAutoLoginTypes::PersistentAuth:
			AccountCredentials.Type = TEXT("persistentauth");
			break;
		case EAutoLoginTypes::DeviceID:
			AccountCredentials.Type = TEXT("deviceid");
			break;
		default:
			AccountCredentials.Type = TEXT("accountportal");
		}
		return System->IdentityInterface->Login(GetPlatformUserIndexForId(PlatformUser), AccountCredentials);
	}
	UE_LOG(LogTemp, Log, TEXT("No EIKSettings found, defaulting to accountportal"));
